...
function test(){
    ...
    //window.location.href = "https://new.storeimg.com/" + document.getElementById("id").value;
    if(hashids.decode(document.getElementById("id").value).length > 0){
        window.location.href = window.location.origin + "/" + document.getElementById("id").value;
    }
    else{
...
class PeersUI {
    ...
    _onJoined(room) {
        ...
        $$('x-code').appendChild(document.createTextNode(room.slug));
...
class QRCode {
    constructor(slug) {
        ...
        const src = `https://api.qrserver.com/v1/create-qr-code/?color=000000&bgcolor=FFFFFF&data=${url.toString()}&qzone=0&margin=15&size=150x150&;ecc=L`;
        img.src = src;
